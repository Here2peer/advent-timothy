...
func inspect(monkey Monkey, in int) Monkey {
	...
	for index, item := range items {
		...
		for point, operation := range str {
			...
			if operation == "old" && point == 2 {
				...
				var val, _ = strconv.Atoi(str[2])
				v2 = int64(val)
			}
		}
		if str[1] == "*" {
			monkey.items[index].worry = int64((v1 * v2))
			monkey.items[index].worry %= int64(div)
	...
	for index, item := range items {
		if monkeys[in].items[index].worry%int64(monkey.test) == 0 {
